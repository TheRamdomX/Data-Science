...
RUTA_PARQUET = os.path.join(BASE_DIR, "Datos_Limpios.parquet")
RUTA_SALIDA  = os.path.join(BASE_DIR, "EDA_Resultados")
os.makedirs(RUTA_SALIDA, exist_ok=True)

columnas_utiles = [
    "Codigo", "RegionUnidad", "sector",
...
columnas_cat = ["RegionUnidad", "sector", "Tipo de Adquisicion",
...
def eda_regiones():
    ...
    guardar_tabla(df, "02_regiones.png", "Regiones")
    ...
    guardar(fig, "02_regiones_bar.png")
    del df; gc.collect()
...
def eda_target():
    ...
    guardar(fig, "04_target_bar.png")
    del df; gc.collect()

for fn in [eda_regiones, eda_sectores, eda_target]:
    fn()

# =========================================================
# SECCION 5 — PEARSON (solo features disponibles al ofertar)
# =========================================================

print("\n=== 5. CORRELACION PEARSON ===")

FEATURES_NUM = [
    "MontoEstimado", "Valor Total Ofertado", "NumeroOferentes",
    "CantidadReclamos", "Target",
]
...
ax.set_title("Correlación de Pearson (Features de Negocio)", fontsize=12)
fig.tight_layout()
guardar(fig, "05_pearson.png")
del corr_matrix; gc.collect()

# =========================================================
# SECCION 6 — CRAMÉR'S V (sin Estado, Anio, Mes)
# =========================================================

print("\n=== 6. CRAMÉR'S V ===")

CATS = [c for c in ["RegionUnidad", "sector", "Tipo de Adquisicion", "Tipo"] if c in schema_cols]
...
plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
ax.set_title("Cramér's V — Asociación Categóricas (sin leakage)", fontsize=12)
fig.tight_layout()
guardar(fig, "06_cramers_v.png")
...
guardar_tabla(pares_df, "06b_cramers_pares.png", "Pares Categóricos por Cramér's V")
del cv_matrix, pares, pares_df; gc.collect()

# =========================================================
# SECCION 7 — ETA² (sin DiasAdjudicacion, sin Estado)
# =========================================================

print("\n=== 7. ETA² ===")

NUMS_ETA = ["MontoEstimado", "Valor Total Ofertado", "NumeroOferentes", "CantidadReclamos", "Target"]
CATS_ETA = [c for c in ["RegionUnidad", "sector", "Tipo de Adquisicion", "Tipo"] if c in schema_cols]
...
plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
ax.set_title("Eta² — Numéricas vs Categóricas (sin leakage)", fontsize=12)
fig.tight_layout()
guardar(fig, "07_eta_squared.png")
del eta_matrix; gc.collect()

# =========================================================
# SECCION 8 — TASA DE ADJUDICACIÓN POR CATEGORÍA (sin Estado)
# =========================================================

print("\n=== 8. TASA DE ADJUDICACIÓN POR CATEGORÍA ===")

for cat in ["RegionUnidad", "sector", "Tipo de Adquisicion", "Tipo"]:
    ...
    guardar_tabla(df, f"08_tasa_{cat.replace(' ','_')}.png", f"Tasa Adjudicación por {cat}")
    ...
    guardar(fig, f"08_tasa_{cat.replace(' ','_')}_bar.png")
    del df; gc.collect()

# =========================================================
# SECCION 9 — RATIO MONTO ADJUDICADO / ESTIMADO
# =========================================================

print("\n=== 9. RATIO MONTO ADJUDICADO / ESTIMADO ===")
...
guardar_tabla(ratio_df, "09_ratio_monto.png", "Ratio Ofertado/Estimado por Sector")
...
ax.set_title("Ratio Monto Ofertado / Monto Estimado por Sector")
ax.legend(); fig.tight_layout()
guardar(fig, "09_ratio_monto_bar.png")
del ratio_df; gc.collect()

# =========================================================
# SECCION 10 — COMPETENCIA (NumeroOferentes)
# =========================================================

print("\n=== 10. COMPETENCIA POR TIPO Y SECTOR ===")
...
for group_col in ["Tipo de Adquisicion", "sector"]:
    ...
    guardar_tabla(df, f"10_competencia_{group_col.replace(' ','_')}.png",
    ...
    guardar(fig, f"10_competencia_{group_col.replace(' ','_')}_bar.png")
    del df; gc.collect()

# =========================================================
# SECCION 11 — ESTADÍSTICAS DESCRIPTIVAS (sin features post-hoc)
# =========================================================

print("\n=== 11. ESTADÍSTICAS DESCRIPTIVAS ===")

COLS_DESC = ["MontoEstimado", "Valor Total Ofertado", "NumeroOferentes", "CantidadReclamos"]
...
guardar_tabla(desc_df, "11_stats_descriptivas.png", "Estadísticas Descriptivas Numéricas")
print(desc_df)
del stats_desc, stats_rows, desc_df; gc.collect()

# =========================================================
# SECCION 12 — NUEVA: Ratio Ofertado/Estimado vs Target
# =========================================================

print("\n=== 12. RATIO OFERTADO/ESTIMADO vs TARGET ===")
...
ratio_target = (
    ...
    .filter(pl.col("Ratio").is_between(0, 5))
    .with_columns(
        (pl.col("Ratio") * 10).cast(pl.Int32).cast(pl.Float64).truediv(10).alias("RatioBin")
    )
    .group_by("RatioBin")
    .agg([
        pl.col("Target").cast(pl.Float64).mean().alias("TasaSeleccion"),
        pl.len().alias("N"),
    ])
    .sort("RatioBin")
    .filter(pl.col("N") >= 100)
...
ax1.bar(ratio_target["RatioBin"].to_list(), ratio_target["TasaSeleccion"].to_list(),
        width=0.08, alpha=0.7, label="Tasa Selección")
ax1.set_xlabel("Ratio Ofertado / Estimado")
ax1.set_ylabel("Tasa de Selección (0-1)")
ax1.axvline(1.0, linestyle="--", color="red", alpha=0.7, label="Ratio = 1")
ax2 = ax1.twinx()
ax2.plot(ratio_target["RatioBin"].to_list(), ratio_target["N"].to_list(),
         "go-", alpha=0.5, label="N observaciones")
ax2.set_ylabel("N observaciones")
ax1.set_title("Tasa de Selección según Ratio Ofertado/Estimado")
lines1, labels1 = ax1.get_legend_handles_labels()
lines2, labels2 = ax2.get_legend_handles_labels()
ax1.legend(lines1 + lines2, labels1 + labels2, loc="upper right")
fig.tight_layout()
guardar(fig, "12_ratio_vs_target.png")
guardar_tabla(ratio_target, "12_ratio_vs_target_tabla.png", "Tasa Selección por Ratio Ofertado/Estimado")
del ratio_target; gc.collect()

# =========================================================
# SECCION 13 — NUEVA: NumeroOferentes vs Target
# =========================================================

print("\n=== 13. NUMERO OFERENTES vs TARGET ===")

oferentes_target = (
    get_base()
    .filter(pl.col("NumeroOferentes").is_not_null() & (pl.col("NumeroOferentes") > 0))
    .with_columns(
        pl.when(pl.col("NumeroOferentes") >= 10).then(pl.lit(10))
        .otherwise(pl.col("NumeroOferentes")).cast(pl.Int32).alias("NumOferentesBin")
    )
    .group_by("NumOferentesBin")
    .agg([
        pl.col("Target").cast(pl.Float64).mean().alias("TasaSeleccion"),
        pl.len().alias("N"),
    ])
    .sort("NumOferentesBin")
    .collect()
)

labels_of = [str(x) if x < 10 else "10+" for x in oferentes_target["NumOferentesBin"].to_list()]

fig, ax1 = plt.subplots(figsize=(10, 6))
ax1.bar(labels_of, oferentes_target["TasaSeleccion"].to_list(), alpha=0.7, label="Tasa Selección")
ax1.set_xlabel("Número de Oferentes")
ax1.set_ylabel("Tasa de Selección (0-1)")
ax2 = ax1.twinx()
ax2.plot(labels_of, oferentes_target["N"].to_list(), "ro-", alpha=0.5, label="N observaciones")
ax2.set_ylabel("N observaciones")
ax1.set_title("Tasa de Selección según Número de Oferentes")
lines1, labels1 = ax1.get_legend_handles_labels()
lines2, labels2 = ax2.get_legend_handles_labels()
ax1.legend(lines1 + lines2, labels1 + labels2, loc="upper right")
fig.tight_layout()
guardar(fig, "13_oferentes_vs_target.png")
guardar_tabla(oferentes_target, "13_oferentes_vs_target_tabla.png", "Tasa Selección por Nro. Oferentes")
del oferentes_target; gc.collect()
...
print("\n" + "="*55)
print("EDA COMPLETO FINALIZADO (enfoque de negocio)")
